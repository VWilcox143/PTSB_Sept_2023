...
/*
    - How many presidents we've had
    - How many are alive
    - Dead
    - Using a forEach loop through the presidents and have it console.log their first name, last name and how old they were when they died.
    -We only want dead presidents to be displayed.
*/
console.log(presidents.length);

const livePres = presidents.filter(
    presidents => 
    presidents.passed === undefined
    );
console.log("Living:", livePres.length);

const deadPres = presidents.filter(
    presidents => 
    presidents.passed !== undefined 
    );
console.log("Dead:", deadPres.length);

deadPres.forEach((deadPres) => 
console.log(deadPres.first, 
            deadPres.last, 
            "died at age", 
            deadPres.passed - deadPres.year));
